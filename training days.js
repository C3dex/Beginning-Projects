const name = 'Nala';
// The scope of `random` is too loose 

const getRandEvent = () => {
  const random = Math.floor(Math.random() * 3);
  if (random === 0) {
    return 'Marathon';
  } else if (random === 1) {
    return 'Triathlon';
  } else if (random === 2) {
    return 'Pentathlon';
  }
};

// The scope of `days` is too tight 
const getTrainingDays = event => {
  let days; 
  if (event === 'Marathon') {
     days = 50;
  } else if (event === 'Triathlon') {
     days = 100;
  } else if (event === 'Pentathlon') {
     days = 200;
  }

  return days;
};


// The scope of `name` is too tight 
const logEvent = (name, event) => {
  console.log(`${name}'s event is: ${event}`);
};

const logTime = (name, days) => {
  console.log(`${name}'s time to train is: ${days} days`);
};

const event = getRandEvent();
const days = getTrainingDays(event);
// Define a `name` variable. Use it as an argument after updating logEvent and logTime 

logEvent(name, event);
logTime(name, days);

const event2 = getRandEvent();
const days2 = getTrainingDays(event2);
const name2 = 'Warren';
 
logEvent(name2, event2);
logTime(name2, days2);

const event3 = getRandEvent();
const days3 = getTrainingDays(event3);
const name3 = 'Joe';
 
logEvent(name3, event3);
logTime(name3, days3);

const event4 = getRandEvent();
const days4 = getTrainingDays(event4);
const name4 = 'Mamma';
 
logEvent(name4, event4);
logTime(name4, days4);

const event5 = getRandEvent();
const days5 = getTrainingDays(event5);
const name5 = 'Jake';
 
logEvent(name5, event5);
logTime(name5, days5);

const event6 = getRandEvent();
const days6 = getTrainingDays(event6);
const name6 = 'Fin';
 
logEvent(name6, event6);
logTime(name6, days6);



